...
export class Annotation extends EventDispatcher {
	constructor (args = {}) {
		...
		for (let action of actions) {
			let elButton = window.$(`<img src="window.$ {action.icon}" class="annotation-action-icon">`);
	...
	installHandles(viewer){
		...
		window.$(viewer.renderArea).append(domElement);


		let annotationStartPos = this.position.clone();
		let annotationStartOffset = this.offset.clone();

		window.$(this.domElement).draggable({
			start: (event, ui) => {
				annotationStartPos = this.position.clone();
				annotationStartOffset = this.offset.clone();
				window.$(this.domElement).find(".annotation-titlebar").css("pointer-events", "none");

				console.log(window.$(this.domElement).find(".annotation-titlebar"));
			},
			stop: () => {
				window.$(this.domElement).find(".annotation-titlebar").css("pointer-events", "");
			},
			drag: (event, ui ) => {
	...
	removeHandles(viewer){
		...
		//window.$(viewer.renderArea).remove(this.handles.domElement);
