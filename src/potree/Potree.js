...
export function loadPointCloud(path, name, callback){
	...
	let promise = new Promise( resolve => {
		...
		if (!path){
			...
			EptLoader.load(path, function(geometry) {
				if (!geometry) {
					console.error(new Error(`failed to load point cloud from URL: window.${path}`));
				}
				else {
	...
	if(callback){
		return promise.then(pointcloud => {
			loaded(pointcloud);
		});
	}else{
		return promise; /*.then(pointcloud => {
			pointcloud.pointcloud.name = name;
			return pointcloud;
		});*/
	}
};
	...
	jq.fn.extend({
		selectgroup: function(args = {}){
			...
			elGroup.find("option").each((index, value) => {
				let buttonID = jq(value).prop("id");
				let label = jq(value).html();
				let optionValue = jq(value).prop("value");
			...
			elButtonContainer.find("label").each( (index, value) => {
				jq(value).css("margin", "0px");
				jq(value).css("border-radius", "0px");
				jq(value).css("border", "1px solid black");
				jq(value).css("border-left", "none");
			});
			elButtonContainer.find("label:first").each( (index, value) => {
				jq(value).css("border-radius", "4px 0px 0px 4px");

			});
			elButtonContainer.find("label:last").each( (index, value) => {
				jq(value).css("border-radius", "0px 4px 4px 0px");
				jq(value).css("border-left", "none");
			});
